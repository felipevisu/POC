...
async function loadQuestions() {
  ...
  els.title.textContent = data.title || "POC Learning Quiz";
  els.startSummary.textContent = data.summary || "";
  state.questions = data.questions || [];
  els.startMeta.textContent = `${state.questions.length} questions · evaluates how well you understood this POC`;
}
...
function submitAnswer() {
  ...
  state.results.push({ question: q.question, isRight });
...
function showResult() {
  ...
  let msg;
  if (pct === 100) msg = "Mastered it — you fully understood this POC. 🎉";
  else if (pct >= 80) msg = "Strong grasp of the POC. 👏";
  else if (pct >= 60) msg = "Decent understanding — review the misses below.";
  else if (pct >= 40) msg = "Partial understanding. Revisit the POC code.";
  else msg = "Worth studying the POC again before retrying.";
  els.resultMessage.textContent = `${pct}% — ${msg}`;

  els.resultReview.innerHTML = "";
  state.results.forEach((r, i) => {
    const li = document.createElement("li");
    const mark = r.isRight
      ? '<span class="mark ok">✓</span>'
      : '<span class="mark no">✗</span>';
    li.innerHTML = `${mark}<span>Q${i + 1}. ${r.question}</span>`;
    els.resultReview.appendChild(li);
  });
}
...
loadQuestions().catch((err) => {
  els.startSummary.textContent = "Error loading quiz questions.";
  console.error(err);
});
